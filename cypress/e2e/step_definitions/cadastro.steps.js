...
Then('o botão {string} deve estar habilitado', (botao) => {
  cy.get('button[type="submit"]').should('be.enabled');
});

When('clico no botão {string}', (botao) => {
  cy.get('button[type="submit"]').click({ force: true });
  cy.wait(1000); // Espera explícita para observar cambios
});

Then('sou redirecionado para {string}', (path) => {
  cy.url().then((currentUrl) => {
    if (!currentUrl.includes(path)) {
      cy.log(`⚠️ **Error**: Se esperaba redirección a "${path}", pero la URL actual es ${currentUrl}`);
    }
  });
});
